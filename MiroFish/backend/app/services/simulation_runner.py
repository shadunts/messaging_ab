...
class SimulationRunner:
    ...
    def get_run_state(cls, simulation_id: str) -> Optional[SimulationRunState]:
        """获取运行状态（始终从文件重新加载，因为子进程会直接更新文件）"""
        # Always reload from file since the subprocess updates run_state.json directly
    ...
    def start_simulation(
        ...
        try:
            ...
            if max_rounds is not None and max_rounds > 0:
                ...
                # 使用max_rounds时不进入等待命令模式，模拟完成后直接退出
                # 这样监控线程能检测到进程退出并标记为completed
                cmd.append("--no-wait")
