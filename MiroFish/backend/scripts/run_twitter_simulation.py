...
class TwitterSimulationRunner:
    ...
    async def run(self, max_rounds: int = None):
        ...
        # 如果指定了最大轮数，则截断并调整 minutes_per_round
        # 确保截断后仍覆盖至少24小时（包含peak和off-peak时段）
        if max_rounds is not None and max_rounds > 0:
            original_rounds = total_rounds
            total_rounds = min(total_rounds, max_rounds)
            if total_rounds < original_rounds:
                # Recalculate minutes_per_round so max_rounds covers at least 24h
                min_hours_needed = 24
                ideal_minutes = (min_hours_needed * 60) // total_rounds
                if minutes_per_round < ideal_minutes:
                    minutes_per_round = ideal_minutes
                    print(f"\n轮数已截断: {original_rounds} -> {total_rounds} (max_rounds={max_rounds}), minutes_per_round调整为{minutes_per_round}以覆盖24h")
        ...
        # 创建环境 - 使用random推荐系统避免twhin-bert在CPU上挂起
        print("创建OASIS环境...")
        ...
        if initial_posts:
            ...
            if initial_actions:
                ...
                print(f"  已发布 {len(initial_actions)} 条初始帖子", flush=True)
        ...
        for round_num in range(total_rounds):
            ...
            print(f"  [Round {round_num}] hour={simulated_hour} active_agents={len(active_agents)}", flush=True)
            ...
            # 执行动作
            print(f"  [Round {round_num}] Calling env.step with {len(actions)} LLM actions...", flush=True)
            await self.env.step(actions)
            print(f"  [Round {round_num}] env.step completed", flush=True)
        ...
        # Update run_state.json to mark simulation as completed
        # Use absolute path to avoid cwd issues
        run_state_path = os.path.abspath(os.path.join(self.simulation_dir, "run_state.json"))
        print(f"  - Updating run_state at: {run_state_path}", flush=True)
        try:
            with open(run_state_path, 'r', encoding='utf-8') as f:
                run_state = json.load(f)
            run_state["runner_status"] = "completed"
            run_state["current_round"] = total_rounds
            run_state["completed_at"] = datetime.now().isoformat()
            # Count actions from DB
            db_conn = sqlite3.connect(db_path)
            cursor = db_conn.cursor()
            cursor.execute("SELECT COUNT(*) FROM trace")
            run_state["total_actions_count"] = cursor.fetchone()[0]
            run_state["twitter_actions_count"] = run_state["total_actions_count"]
            cursor.execute("SELECT COUNT(*) FROM post")
            posts = cursor.fetchone()[0]
            db_conn.close()
            run_state["progress_percent"] = 100.0
            ...
            print(f"  - run_state.json updated: completed, {run_state['total_actions_count']} actions, {posts} posts", flush=True)
            # Verify
            with open(run_state_path, 'r') as f:
                verify = json.load(f)
            print(f"  - Verified: runner_status={verify['runner_status']}", flush=True)
        except Exception as e:
            import traceback
            print(f"  - ERROR updating run_state.json: {e}", flush=True)
            traceback.print_exc()
